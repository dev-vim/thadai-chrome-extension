...
test('popup without config', async ({ page, extensionId }) => {
  ...
  await expect(page.locator('#popup-banner-message')).toHaveText('Kindly set up your configuration in the settings to proceed');
});
...
test('popup after set config', async ({ page, context, extensionId }) => {
  ...
  await settingsPage.fill('#private-key-input', '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80');
  ...
  await settingsPage.fill('#thadai-contract-address-input', '0xAD523115cd35a8d4E60B3C0953E0E0ac10418309');  await settingsPage.click('#save-settings-btn');
...
test('popup when is rpc unavailable', async ({ page, extensionId }) => {
  ...
  await page.reload();
  await expect(page.locator('#popup-banner-message')).toHaveText('Unable to connect to the blockchain network - check connection or configuration');
});
